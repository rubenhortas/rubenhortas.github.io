...
Each plaintext message is broken into 256 byte blocks. For each block of plaintext, a corresponding 256 byte block from the book is used as the key,
starting from the first chapter, and progressing. No part of the book is ever re-used as key. The use of a key of the same length as the plaintext,
and only using it once is called a “One Time Pad”.  
  
Look in the krypton6 directory. You will find a file called ‘plain1’, a 256 byte block. You will also see a file ‘key1’, the first 256 bytes of ‘A Tale of Two Cities’.
...
Choosing keys that are the same size as the plaintext is impractical. Therefore, other methods must be used to obscure ciphertext against frequency analysis
in a simple substitution cipher. The impracticality of an ‘infinite’ key means that the randomness, or entropy, of the encryption is introduced via the method.  
...
Until now, **we have been playing with classical ciphers, approximating ‘block’ ciphers**. A block cipher is done in **fixed size blocks** (suprise!). 
...
**Its time to employ a stream cipher**. A stream cipher attempts to create an on-the-fly **‘random’ keystream to encrypt the incoming plaintext one byte at a time**.
Typically, **the ‘random’ key byte is xor’d with the plaintext to produce the ciphertext**. **If the random keystream can be replicated at the recieving end,
...
* If the random keystream (k) can be replicated at the recieving end, then a further xor will produce the plaintext once again → k ⊕ pt = ct → pt ⊕ ct = k
...
krypton6@bandit:/tmp/rubenhortas$ python3 -c "print('0'*50)" > /tmp/rubenhortas/zeros_pt.txt
...
We'll get the keys xoring the plaintext with the ciphertext: 
...
The key will depend on the input (or the plaintext).

I we take a closer look to the plaintexts and the ciphertext we can see that when we increment by one character our input (or plaintext),
the output (or ciphertext) will also increased by one character.
...
We can see that when encripting a plaintext character in a certain position, it always give the same encrypted character in the same position...

So I did a 26x30 matrix encrypting every alphabet character 30 times (the length of the LFSR period).
And the plaintext password for the krypton7 level will be the result of find the plaintext character corresponding to the encrypted character in our matrix:
